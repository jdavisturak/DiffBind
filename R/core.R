...
pv.vectors <- function(pv,mask,minOverlap=2,attributes,bAllSame=F,
                       merge=TRUE){
    ...
    if(!bAllSame) {
        if(merge) {
            allpeaks <- bind_rows(lapply(peaks,function(x)x[,1:3]))
        } else {
        ...
        colnames(result) <- c("CHR","START","END",pv$class[PV_ID,1:numvecs])
        pv$binding <- result
...
pv.consensus <- function(pv,sampvec,minOverlap=2,bFast=F,sampID){
    ...
    goodvecs <- apply(tmp$binding[,4:ncol(tmp$binding)],1,pv.minOverlap,minOverlap)
    
    tmp$binding  <- tmp$binding[goodvecs,]
    
    mean.density <- apply(tmp$binding[,4:ncol(tmp$binding)],1,pv.domean)
    ...
    pv <- pv.peakset(pv,peaks=tmp$binding,
                     sampID       =  sampID,
...
pv.plotPCA <- function(pv,attributes=PV_ID,second,third,fourth,size,mask,
    ...
    if(b3D) {
        if (requireNamespace("rgl",quietly=TRUE)) {
            rgl::plot3d(pc$rotation[,c(startComp,startComp+2,startComp+1)],col=pv.colorv(classvec,vColors),type='s',size=sval,
...
pv.overlap <- function(pv,mask,bFast=F,minVal=0) {
    ...
    maskA <- pv$called[,A]
    maskB <- pv$called[,B]   
    if(length(peaksets)>=3) maskC <- pv$called[,C]
    if(length(peaksets)==4) maskD <- pv$called[,D]         
    ...
    if(length(peaksets)<4){
        if(length(peaksets)<3) {
            ...
            res <- pv.contrast3(pv$binding,A,B,C,v1=maskA,v2=maskB,v3=maskC)
        }
    } else {
        res <- pv.contrast4(pv$binding,A,B,C,D,v1=maskA,v2=maskB,v3=maskC,v4=maskD) 
    }
...
pv.overlapRate <- function(pv,mask=mask) {
    ...
    res <- sapply(1:length(pv$peaks),function(x)sum(sums>=x))
    return(res)    
}
